...
static void sendSummarizedField(const Query *q, const RSSearchRequest *req, RedisModuleCtx *ctx,
  ...
  for (size_t ii = 0; ii < numIovBufs; ++ii) {
    ...
    size_t lastSize = Buffer_Offset(&bufTmp);
    ...
    size_t newSize =
        stripDuplicateSpaces(Buffer_GetData(&bufTmp) + lastSize, Buffer_Offset(&bufTmp) - lastSize);
    Buffer_SetOffset(&bufTmp, lastSize + newSize);
